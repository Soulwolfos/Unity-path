...
public class PathFinder : MonoBehaviour
{
    ...
    public void FindPath() {
        NativeHashMap<int2, bool> isObstacle = new NativeHashMap<int2, bool>(obstacles.Count, Allocator.TempJob);
        ...
        for (int i = 0; i < nodeArray.Length; i++) {
            Vector3Int currentNode = new Vector3Int(nodeArray[i].coord.x, nodeArray[i].coord.y, 0);
            if (!start.coord.Equals(nodeArray[i].coord) && !end.coord.Equals(nodeArray[i].coord) && !obstacles.ContainsKey(nodeArray[i].coord)) {
        ...
        if (nodes.ContainsKey(end.coord)) {
            ...
            while (!currentCoord.Equals(start.coord)) {
                ...
                map.SetColor(currentTile, Color.yellow);

                if (currentCoord.Equals(start.coord)) {
                    //el ultimo en pintar es el inicio por lo que lo dejo en verde para identificarlo
        ...
        public void Execute() {
            ...
            do {
                ...
                for (int i = 0; i < offsets.Length; i++) {
                    if (!nodes.ContainsKey(current.coord + offsets[i]) && !isObstacle.ContainsKey(current.coord + offsets[i])) {
                        Node neighbour = new Node {
                            coord = current.coord + offsets[i],
                            parent = current.coord,
                            gScore = current.gScore + SquaredDistance(current.coord, current.coord + offsets[i]),
